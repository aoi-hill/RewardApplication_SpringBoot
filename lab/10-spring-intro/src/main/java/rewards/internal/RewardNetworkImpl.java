...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		...
		Account acc = accountRepository.findByCreditCard(dining.getCreditCardNumber());
		Restaurant res = restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());
		MonetaryAmount amt = res.calculateBenefitFor(acc, dining);
		AccountContribution cur = acc.makeContribution(amt);
		RewardConfirmation conf = rewardRepository.confirmReward(cur, dining);
		accountRepository.updateBeneficiaries(acc);
